  ...
  if (isMobile) {
    return (
      <div className="p-5 sticky top-0 left-0 z-50">
        {!isMobileOpen ? (
          <MenuIcon className=""  color="#ffff" onClick={() => setIsMobileOpen(true)} />
        ) : (
            ...
            <div className="mt-5 flex flex-col gap-5 h-full text-center items-center pt-60">
              {headerRoutes.map((route) =>
                route?.button ? (
                  <Button
                    key={route.href}
                    className="hover:bg-white text-white group w-max"
                  >
                    <Link
                  ...
                  <span
                    className="text-lg font-light hover:text-white text-white cursor-pointer select-none"
                    key={route.href}
                    onClick={() => {
  ...
  return (
    <header className="px-4 lg:px-6 h-14 flex items-center max-w-screen-xl mx-auto w-full text-primary py-10 sticky top-0 backdrop-blur-sm z-50">
      <Link className="flex items-center justify-center" href="#">
        ...
        <span className="ml-2 text-white">Scrape X</span>
      </Link>
